...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-center">
        <div class="card col-md-6">
            <div class="card-header h2">Edit Contact</div>
            <div class="card-body pt-4">
                <form class="form fs-4" action="{{ route('update', [$viewData['edited']->getId()] ) }}" method="post">
                    @csrf
                    @method('PUT')
                    <div class="form-group input-group mb-3">
                        <label class="input-group-text fs-4 col-3 ps-4" for="name">Name</label>
                        <input type="text" name="name" id="name" class="form-control" value="{{ $viewData['edited']->getName() }}">
                    </div>
                    <div class="form-group input-group mb-3">
                        <label class="input-group-text fs-4 col-3 ps-4" for="num">Number</label>
                        <input type="text" name="num" id="num" class="form-control" value="{{ $viewData['edited']->getNum() }}" required>
                    </div>
                    <div class="form-group input-group mb-4">
                        <label class="input-group-text fs-4 col-3 ps-4" for="mail">mail</label>
                        <input type="text" name="mail" id="mail" class="form-control" value="{{ $viewData['edited']->getMail() }}">
                    </div>
                    <div class="form-group input-group mb-4 d-flex">
                        <div class="input-group-text">
                            <input type="checkbox" name="favorite" id="favorite" class="col" {{ $viewData['edited']->getFavorite()=='yes' ? 'checked':'' }}>
                        </div>
